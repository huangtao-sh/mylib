...
if __name__=='__main__':
    pwd='123456'
    with open('d:/a.txt','w')as fn:
        fn.write(make_password(pwd))

    for i,hasher in enumerate(HASHERS):
        enc=make_password(pwd,hasher.algorithm)
        result='Success' if check_password(pwd,enc) else 'Failed'
        print('%d:%s\t%s'%(i,hasher.algorithm,result))
        print(enc)
